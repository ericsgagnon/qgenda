...
func TestToIndirectReflectValue(t *testing.T) {
	...
	var testCases = []struct {
		...
		{Name: "qgenda.Schedule", Input: schedule, Expect: Expect{Value: reflect.ValueOf(schedule), Type: reflect.TypeOf(schedule), Pointer: false}},
	}
	...
	tschedule, err := meta.ToStruct(schedule)
	if err != nil {
		log.Println(err)
	}
	fmt.Println(tschedule)
